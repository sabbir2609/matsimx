...
# Corrdump command: corrdump -l=rndstr.in -ro -noe -nop -clus -2=1.2 -3=1.0
# Total sites in supercell: 64
# Parallel runs per concentration: 5
# Maximum concurrent jobs: 6
# Concentrations: [0.125, 0.25, 0.015625, 0.375, 0.625, 0.984375, 0.4375, 0.5, 0.75]
# Total estimated time: 184.5 minutes

set -e

SWEEP_ELEMENT="Zn"
COMPLEMENT_ELEMENT="Mn"
TIME_PER_CONC_DEFAULT=20.5
MAX_PARALLEL=6
PARALLEL_RUNS_PER_CONC_DEFAULT=5
CORRDUMP_CMD="corrdump -l=rndstr.in -ro -noe -nop -clus -2=1.2 -3=1.0"
...
echo "🔍 Concentrations to be searched: [0.125, 0.25, 0.015625, 0.375, 0.625, 0.984375, 0.4375, 0.5, 0.75]"
...
run_concentration() {
    ...
    local current_run=$2
    local total_runs=$3
    local sweep_atoms=$(printf "%.0f" $(echo "$conc * 64" | bc))
    local comp_atoms=$(echo "64 - $sweep_atoms" | bc)
